...
//This funcion checks and clamps the jount angles to a safe zone
vector<float> clamp_at_boundaries(float requested_j1, float requested_j2) {
  //Define clamped joint angles and assign them to be requested ones 
  float clamped_j1 = requested_j1;
  float clamped_j2 = requested_j2;

  //Get min and max joint parameters, andd assigning them to their respective
  float min_j1{0.0f}, max_j1{0.0f}, min_j2(0.0f), max_j2{0.0f};
  ...
  //Check if joint 1 falls in th esafe zone, otherwise clamp it 
  if(requested_j1 < min_j1 || requested_j1 > max_j1) {
    clamped_j1 = std::min(std::max(requested_j1, min_j1), max_j1);
    ROS_WARN("j1 is out of bounds, valid range (%1.2f,%1.2f), clamping to: %1.2f\n", min_j1, max_j1, clamped_j1);
  }

  //Check if joint 2 falls in th esafe zone, otherwise clamp it
  if(requested_j2 < min_j2 || requested_j2 > max_j2) {
    clamped_j2 = std::min(std::max(requested_j2, min_j2), max_j2);
    ROS_WARN("j2 is out of bounds, valid range (%1.2f,%1.2f), clamping to: %1.2f\n", min_j2, max_j2, clamped_j2);
  }

  vector<float> clamped_data{clamped_j1, clamped_j2};
  return clamped_data;
}

//This callback function executes whenever a safe move service is requested 
bool handle_safe_move_request(simple_arm::GoToPosition::Request &req, 
   simple_arm::GoToPosition::Response &res) {
  ROS_INFO("GoToPositionRequest received - j1:%1.2f, j2:%1.2f", req.joint_1, req.joint_2);

  //check if the requested joint angles are in the safe zone, otherwise clamp it 
  vector<float> joint_angles = clamp_at_boundaries(req.joint_1, req.joint_2);
...
int main(int argc, char **argv) {
  //Initialize the amr mover node and create a handle to it 
  ros::init(argc, argv, "arm_mover");
  ros::NodeHandle n;

  //Define 2 publishers to publish std::msgs::Float64 messages on joints respective topics 
  ...
  //Define a safe_move service with a handle_safe_move_request callbakc function 
